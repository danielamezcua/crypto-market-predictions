...
dataset_btc = pd.read_csv("./datasets/ETH.csv")
n = len(dataset_btc)
x = []
for i in range(0,n):
...
x = dataset_btc.loc[:, ["open","close","high","low","avg_compound_ns"]].values
...
x = dataset_btc.loc[:, ["open","close","high","low","avg_compound_s"]].values
y = dataset_btc.loc[:, "label"].values
